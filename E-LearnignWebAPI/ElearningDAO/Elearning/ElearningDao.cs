    ...
    public class ElearningDao
    {
        ...
        public DataTable GetAssignmentByEmail(string username)
        {
            ...
            try
            {
                ...
                SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DevConnection"].ConnectionString);
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT * FROM EL_GetAssignmentByEmail(@p_username)", conn);
                cmd.Parameters.AddWithValue("@p_username", username);
                SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                adapter.Fill(dt);
            }
            ...
            {
                throw new Exception("ElearningDao > GetAssignmentByEmail Error: " + ex.Message);
            }
            return dt;
        }
    }
